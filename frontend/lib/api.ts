import type { AdminMetrics, Assignment, Participant, SubmissionResult, User } from './types'
...
export async function getAdminMetrics(token: string) {
  ...
  return response.ok ? await response.json() as AdminMetrics : { totalParticipants: 0, registered: 0, checkedIn: 0, active: 0, completed: 0, connected: 0, inContest: 0, submitted: 0, disconnected: 0, currentRound: null, liveContestStatus: 'UNKNOWN', countdownSeconds: 0 }
}
